...
; The above defines make-state, state-m, and state-e
...
(define (next-state s)
  (let ([m (state-m s)]
    ...
    (cond
     ...
     ;; Rule 3 (two cond lines):
     [(and (primapp? m)
	   (not (val? (primapp-arg1 m))))
      (make-state (primapp-arg1 m)
		  (replace e hole (make-primapp (primapp-op m) 
                                                hole 
                                                (primapp-arg2 m))))]
     [(and (primapp? m)
     ...
     ;; Remaining rules depend on the expression around the 
     ;;   hole in the context:
     [(val? m)
...
(define (substitute m x v)
  (cond
   ...
   [(func? m) 
    (if (eq? x (func-var m))
	m
	;; First, rename formal var, if necessary:
	(let ([m2 (if (member (func-var m) (free-vars v null))
		      ;; rename:
		      (let ([v2 (gensym)])
			(make-func v2
				   (substitute (func-body m) (func-var m) v2)))
		      ;; no rename needed:
		      m)])
...
(define (free-vars m bound)
  (cond
   [(symbol? m) (if (memq m bound)
		    ...
		    (list m))]
   [(app? m)
    (append (free-vars (app-func m) bound)
	    (free-vars (app-arg m) bound))]
   [(primapp? m)
    (append (free-vars (primapp-arg1 m) bound)
...
(define (hole-expression e)
  (cond
   [(app? e)
    (cond
     ;; ([] M) or (M []) => that's the answer
     [(or (hole? (app-func e)) (hole? (app-arg e))) e]
     ;; Look for wrapped hole in either func or arg part
     [else
...
(define (replace e old new)
  (cond
   [(equal? e old) new]
   [(app? e) (make-app (replace (app-func e) old new)
		       (replace (app-arg e) old new))]
   [(primapp? e) (make-primapp (primapp-op e)
   ...
   [(number? e) e]
   [(symbol? e) e]
   [else (error 'replace "ill-formed expression: ~e" m)]))
